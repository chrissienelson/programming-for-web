...
function () {
 function SoundFile(paths, onload, onerror, whileLoading) {
   ...
   this.panner = new panner_0(this.output, master.input, 2); 
...
function (_Oscillator) {
 ...
 function Pulse(freq, w) {
   ...
   _this.dNode = master.audiocontext.createDelay(); 

   _this.dcOffset = createDCOffset();
   _this.dcGain = master.audiocontext.createGain();
...
function (_AudioVoice) {
 ...
 function MonoSynth() {
   ...
   master.soundArray.push(monosynth_assertThisInitialized(_this));
   /**
    * Getters and Setters
